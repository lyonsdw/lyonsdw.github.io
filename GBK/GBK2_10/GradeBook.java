...
public class GradeBook {
  ...
  private final static String VERSION ="gbk v2.10";

  private final static int MAX_STUDENT_COUNT = 500;
  private static boolean DROP_LOWEST = true;
  // variable INDIVHISTPRINT switches on/off whether indiv grade reports get class histogram
  private static boolean INDIVHISTPRINT = false;
  // private static boolean INDIVHISTPRINT = false;
  ...
  private static int  section1Count;           // number of students in section 1 of multisection course
  private static int  section2Count;           // number of students in section 2 of multisection course
  ...
  public static void main(String[] args) {
	...
	currentClass.writeAbsenceReport();	
	currentClass.writeAttendList();
	currentClass.writeXfigAttendList();
	currentClass.makeEmailList();
	currentClass.writePermutedRoster();
	//        currentClass.writeHist(histGroupChoice, histItemChoice);
	System.exit(0);
      }
      break;
    ...
    case 3:
      ...
      // arguments: -s filename sizea size b
    case 4:
      currentClass = new GradeBook(inFileName);
      currentClass.writePermutedSplitRoster();
      System.exit(0);
    }
  ...
  private void menu(){
    ...
    System.out.println(menuLine);
    menuLine = "C  write individual grade reports for student Conferences";
    System.out.println(menuLine);
    menuLine = "S  Send individual grade reports to students";
    System.out.println(menuLine);
    //  gbk now generates a class email list on each run, so this menu item is no longer needed
    //    menuLine = "L  Make a class email list";
    //    System.out.println(menuLine);
    ...
    case 'S': case 's':
      System.out.println("Really send individual grade reports by email?[y/n] ");
      choice = KeybdIn.readLineNonwhiteChar();
      switch(choice){
      case 'Y': case 'y':
	this.sendIndivReports();
        System.exit(0);
      default:
	menu();
      }
    ...
    default:
      System.out.println("Please enter a valid choice (E,W,H,N,Q, or S)\n");
      menu();
    }
  }          // end of menu()
  ...
  public GradeBook(String filename){
    ...
    //    debug note 4/2010 this line probably does not have any effect
  ...
  // assessment seed data : name and letter grade only, csv fields
  public void writeAssessmentSeedReport(){
    // output file name
    String assessmentFile = "gbk.assessment.txt";    
  ...
  private void writePostReport(){
	...
	//round(100*student[i].getOutOfOne(j,k))
	if (ClassStats.round(100*student[i].getOutOfOne(0,0))==j){
	  line = student[i].writePostReport(assignmentList);
	  if ( line == "PRIV"){
	    ;
	  }
	  else {
    ...
    /****** random permutation retired 11/2006, replaced by ordered list ***/
    // student records
  ...
  // permuted list for small groups 
  private void writePermutedRoster(){
    if (studentCount<6){return;} // do not make a permuted list of fewer than 6 students
    String rosterPermutedFile = "gbk.permutelist.txt";
    ...
    line = TextUtil.centerText(line, OUTPUT_COLUMNS) + "\n\n";
    outDataBuffer += line;
    //    line = "Date: " + dateStamp; // remove date stamp 8/30/2017
    // line += "\n\n";
    //    outDataBuffer += line;
    ...
    alpha = RandomPerm.randomPerm(studentCount);
    int offset = 0;
    if (studentCount%4 == 2){offset = 1;}
    if (studentCount%4 == 1){offset = 2;}
    int lastgrp4index = 4*(studentCount/4-offset);
    for (int i=1; i<=lastgrp4index; i++){
      line = student[alpha[i]].getNameAndPresentationCount();
      /*
      // 9/7/2017 new feature that counts presentations from student comments record
      presentationCount = student[alpha[i]].getPresentationCount();
      //      System.out.println("presentation count is " + Integer.toString(presentationCount) + "\n");
      if (presentationCount>0){line += " (" + Integer.toString(presentationCount) + ")";}
      */
        outDataBuffer += line + "\n";
	if (i%4 == 0){                           // make random groups of size 4 8/25/2017
	outDataBuffer += "\n";
	      }
    }
    for (int i=lastgrp4index+1; i<=studentCount; i++){
      line = student[alpha[i]].getNameAndPresentationCount();
      /*
      presentationCount = student[alpha[i]].getPresentationCount();
      //      System.out.println("presentation count is " + Integer.toString(presentationCount) + "\n");
      ...
      */
        outDataBuffer += line + "\n";
	if ((i-lastgrp4index)%3 == 0){                           // make random groups of size 3 after 4s run out
	outDataBuffer += "\n";
	      }
    }
    
    writeOutFile(rosterPermutedFile);
  }

  // split permuted list for small groups, multi-section roster
  private void writePermutedSplitRoster(){
    String rosterPermutedFile = "gbk.permutelist.txt";
    int presentationCount = 0;
    ...
    // use randomPerm() to mix up the order of students
    int[] alpha = new int[studentCount + 1];
    alpha = RandomPerm.randomPermSplit(section1Count,section2Count);
    int offset = 0;
    if (section1Count%4 == 2){offset = 1;}
    if (section1Count%4 == 1){offset = 2;}
    int lastgrp4index = 4*(section1Count/4-offset);
    for (int i=1; i<=lastgrp4index; i++){
      line = student[alpha[i]].getNameAndPresentationCount();
      /*
      // 9/7/2017 new feature that counts presentations from student comments record
    ...
    for (int i=lastgrp4index+1; i<=section1Count; i++){
      line = student[alpha[i]].getNameAndPresentationCount();
      /*
      presentationCount = student[alpha[i]].getPresentationCount();
      //      System.out.println("presentation count is " + Integer.toString(presentationCount) + "\n");
    ...
    outDataBuffer += "\f"; // put section b permuted roster on new page
    line = courseTitle;
    line = TextUtil.centerText(line, OUTPUT_COLUMNS) + "\n\n";
    outDataBuffer += line;
    
    int offset2 = 0;
    if (section2Count%4 == 2){offset2 = 1;}
    if (section2Count%4 == 1){offset2 = 2;}
    lastgrp4index = section1Count + 4*(section2Count/4-offset2);
    for (int i=section1Count+1; i<=lastgrp4index; i++){
      line = student[alpha[i]].getNameAndPresentationCount();
      /*
      // 9/7/2017 new feature that counts presentations from student comments record
      presentationCount = student[alpha[i]].getPresentationCount();
      ...
      */
        outDataBuffer += line + "\n";
	if ((i-section1Count)%4 == 0){                           
	outDataBuffer += "\n";
	      }
    }
    for (int i=lastgrp4index+1; i<=section2Count; i++){
      line = student[alpha[i]].getNameAndPresentationCount();
      /*
      presentationCount = student[alpha[i]].getPresentationCount();
      //      System.out.println("presentation count is " + Integer.toString(presentationCount) + "\n");
  ...
  // individual grade reports to send via email
  private void sendIndivReports(){
    String indivReportFile = "gbk.indiv_report.txt";
    String indivReportFileTop = "gbk.indiv_report_top.txt";
    String indivReportFileBottom = "gbk.indiv_report_bottom.txt";
    ...
    // header, title
    line = "Grade Report: DO NOT REPLY to this email \n\n";
    outDataBuffer += line;
    line = courseTitle + "\n";
    //    line = TextUtil.centerText(line, OUTPUT_COLUMNS) + "\n\n";
    outDataBuffer += line;
    ...
    writeOutFile(indivReportFileTop);
    outDataBuffer = "";
    ...
    /* // suppress assignments and grade scale from reports sent to students 12/2016 */
    /* line = "********* ASSIGNMENTS and GRADE SCALE *********"; */
    /* line = TextUtil.centerText(line, OUTPUT_COLUMNS) + "\n\n"; */
    /* outDataBuffer += line; */
    ...
    // cumulative histogram
    // if final exam has been entered, do not print histogram
    // switch this global variable INDIVHISTPRINT then recompile to enable or disable histogram on indiv email report
    if ((assignmentList.getGroupSize()[assignmentList.getGroupCount()] == 0) && INDIVHISTPRINT){
      line = "*************** HISTOGRAM ***************";
      line = TextUtil.centerText(line, OUTPUT_COLUMNS) + "\n\n";
      outDataBuffer += line;
      //      outDataBuffer += classStats.writeHorizHist(0,0,assignmentList);
      outDataBuffer += classStats.writePostHist();//vertical format histogram
    }
    outDataBuffer += "\n\n";
    writeOutFile(indivReportFileBottom);
    outDataBuffer = "";
    ...
    // student record
	  ...
	  if (email.length()>0){
	  line = student[i].writeEmailGradeReport(assignmentList);
	    outDataBuffer += line + "\n\n";
	    writeOutFile(indivReportFile);
	    // system call here to script to assemble files and mail
	    try{
	      command ="sendem " + email;
	      Process child = Runtime.getRuntime().exec(command);
	      try{
		int childcount =  child.waitFor();
	      }
	...
	try{
	  int childcount2 =  child2.waitFor();
	}
	catch(InterruptedException e){}
      }
      catch(IOException e){}
  ...
  // make a class email list
  private void makeEmailList(){
    String EmailListFile = "gbk.email_list.txt";
...
// student record
  ...
  writeOutFile(EmailListFile);
  }

  // individual grade reports to write to a file for student conferences
  private void writeIndivReports(){
    String indivReportFile = "gbk.indiv_report.txt";
    String indivReportFileTop = "gbk.indiv_report_top.txt";
    String indivReportFileBottom = "gbk.indiv_report_bottom.txt";
    ...
    // cumulative histogram
    // if final exam has been entered, do not print histogram
    if (assignmentList.getGroupSize()[assignmentList.getGroupCount()] == 0){
      line = "*************** HISTOGRAM ***************";
      line = TextUtil.centerText(line, OUTPUT_COLUMNS) + "\n\n";
      outDataBuffer += line;
      outDataBuffer += classStats.writeHorizHist(0,0,assignmentList);
    ...
    // student record
	  ...
	  email = student[i].getEmail();
	  if (email.length()>0){
	  line = student[i].writeGradeReport(assignmentList);
	    outDataBuffer += line + "\n\n";
	    writeOutFile(indivReportFile);
	    // system call here to script to assemble files and mail
	    try{
	      command ="writeconfem " + student[i].getName();
	      Process child = Runtime.getRuntime().exec(command);
	      try{
		int childcount =  child.waitFor();
	      }
  ...
  private void writeHist(int groupChoice, int itemChoice){
    ...
    // 12/2012 write a list of scores for the chosen assignment in a single file in a text file
    outDataBuffer = "";
    for (int i=1; i<=studentCount; i++){
      outDataBuffer += ClassStats.round(student[i].outOfOne[groupChoice][itemChoice]*100)+ "\n";
	  writeOutFile(scoreListFile);
    }
  }

  // absence report file, added 9/12/2017
  private void writeAbsenceReport(){
    String listFile = "gbk.absences.txt";
    String line = "";
    int splitLoc = 0;
    int mark = 0;
    ...
    for (int i=1; i<=studentCount; i++){
      line = "       " + student[i].getAbsenceCount() + "  " + student[i].getName();
      if (i%5 == 0){
	line += "\n";
      }
      outDataBuffer += line + "\n";
  ...
  private static int readArgs(String[] args){
    ...
    if (args[0].equals("-s")){ // args 2 and 3 are sizes sections in a 2-section roster
      inFileName = args[1];
      section1Count = Integer.parseInt(args[2]);
      section2Count = Integer.parseInt(args[3]);
      return 4;
    }    
    else {
      return -1;
